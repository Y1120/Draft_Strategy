...
def fetch_historical_data(exchange, symbol_x):
    try:
        ########## fetch symbol x data ##########
        ...
        while since < end:
            try: 
                candles = exchange.fetch_ohlcv(symbol_x,timeframe='1m',since=since,limit=100)
                if not candles:
...
def fetch_merge_data(symbol_x,symbol_y):
    symbol1 = symbol_x.replace("-USDT-SWAP","").lower()
...
def main():
    symbol1 = 'AI16Z-USDT-SWAP'
    symbol2 = 'VIRTUAL-USDT-SWAP'
    exchange = ccxt.okx({
    ...
    pairs_trade = Pairs_trade(exchange = exchange, symbol_x = symbol1, symbol_y = symbol2)

    while True:
        fetch_historical_data(exchange, symbol1)
        fetch_historical_data(exchange,symbol2)
        fetch_merge_data(symbol1,symbol2)
        pairs_trade.get_data()
        pairs_trade.calculate_spread()
        pairs_trade.exit_position()
