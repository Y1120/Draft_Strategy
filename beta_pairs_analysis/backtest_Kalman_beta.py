...
def backtest1(symbol1,symbol2):
    ...
    for i in range(len(merage_data)):
        ...
        if entry_time is None:
            ...
            entry_beta = beta.loc[beta['timestamp'] == entry_time, 'beta'].iloc[0]    
        print("entry_beta",entry_beta)
        spread = merage_data['spread'].iloc[i]
        # Update trading positions
        ai_position, vr_position, PNL, is_exit, entry_time,amount1,amount2 = exit_position(
    ...
    print("ai_close",len(ai_close))
    print("beta",len(beta))
    num = len(timestamps)
    beta1 = np.array(beta['beta'][-num:])
    results = {
    ...
    results.to_csv("C:\\Users\\theo\\Desktop\\Kalman_0.75std_E.csv")
...
def plot_trading_results(timestamps, pnl, ai_pos, vr_pos, ratio, upper_bounds, lower_bounds, means,symbol1,symbol2,symbol1_close,symbol2_close):
    ...
    print("plot_ai_pos",ai_pos)
    print("plot_vr_pos",vr_pos)
    print("plot_ratio",ratio)
    ...
    file_path = os.path.join(folder_path, f'Kalman_0.75_E.png')
