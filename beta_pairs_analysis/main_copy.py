...
async def fetch_historical_data(exchange, symbol_x):
    try:
        ...
        while since < end:
            try:
                candles = await run_in_executor(exchange.fetch_ohlcv, symbol_x, '1m', since, 100)
                if not candles:
...
async def process_pair(exchange, symbol_x, symbol_y):
    ...
    while True:
        ...
        await fetch_merge_data(symbol_x, symbol_y)
        await pairs_trade.get_data()
        await pairs_trade.calculate_spread()
        await pairs_trade.exit_position()
...
async def main():
    ...
    pairs = [["1INCH-USDT-SWAP", "CORE-USDT-SWAP"], ["ACE-USDT-SWAP", "CSPR-USDT-SWAP"],["AI16Z-USDT-SWAP", "VIRTUAL-USDT-SWAP"]]
